...
local function VersionParts(version)
    local startPos, endPos, major, minor, patch = string.find(version or "", "^(%d+)%.(%d+)%.(%d+)")
    return tonumber(major) or 0, tonumber(minor) or 0, tonumber(patch) or 0
end

function OTLGM:IsVersionNewer(left, right)
    local la, lb, lc = VersionParts(left)
    local ra, rb, rc = VersionParts(right)
...
local BaseEnsureDB = OTLGM.EnsureDB
local BaseGetGuildDB = OTLGM.GetGuildDB
local BaseMigrateGuildDB = OTLGM.MigrateGuildDB
...
function OTLGM:GetUnreadSummary()
    local result = { total = 0, joins = 0, leaves = 0, ranks = 0, levels = 0, returns = 0, notes = 0 }
    local db = self:GetGuildDB()
    if not db then return result end
    local i, eventInfo
    for i = 1, table.getn(db.log or {}) do
        eventInfo = db.log[i]
        if eventInfo and not eventInfo.reviewed and not eventInfo.hiddenLegacyLevel then
            result.total = result.total + 1
            if eventInfo.kind == "JOIN" then result.joins = result.joins + 1 end
            if eventInfo.kind == "LEAVE" then result.leaves = result.leaves + 1 end
            if eventInfo.kind == "RANK" then result.ranks = result.ranks + 1 end
            if eventInfo.kind == "LEVEL" then result.levels = result.levels + 1 end
            if eventInfo.kind == "RETURN" then result.returns = result.returns + 1 end
            if eventInfo.kind == "NOTE" then result.notes = result.notes + 1 end
        end
    end
    return result
end
...
function OTLGM:PromoteMember(name)
    if not self:CanPromoteMembers() then self:ShowPermissionNotice("PROMOTE") return end
...
function OTLGM:DemoteMember(name)
    if not self:CanDemoteMembers() then self:ShowPermissionNotice("DEMOTE") return end
...
function OTLGM:RemoveMember(name)
    if not self:CanRemoveMembers() then self:ShowPermissionNotice("REMOVE") return end
...
function OTLGM:HandleAddonMessage(prefix, message, channel, sender)
    ...
    startPos, endPos, version = string.find(message, "^Q|(.+)$")
    if version then
        self:RememberAddonUser(sender, version)
        local now = self:Now()
...
function OTLGM:ExportBackup()
    ...
    for i = 1, table.getn(settingsKeys) do
        key = settingsKeys[i]
        table.insert(lines, "S|" .. key .. "|" .. EscapeField(OTLGM_DB.settings[key]))
    end
    local name, flags
    for name, flags in pairs(db.memberFlags or {}) do
        table.insert(lines, "F|" .. EscapeField(name) .. "|" .. EscapeField(flags.inactiveStatus) .. "|" .. tostring(flags.inactiveStatusAt or 0) .. "|" .. EscapeField(flags.inactiveStatusBy))
    end
    local eventInfo
    for i = table.getn(db.log or {}), 1, -1 do
        eventInfo = db.log[i]
        table.insert(lines, table.concat({
            "E", tostring(eventInfo.ts or 0), EscapeField(eventInfo.kind), EscapeField(eventInfo.name),
            EscapeField(eventInfo.detail), EscapeField(eventInfo.actor), EscapeField(eventInfo.source),
            EscapeField(eventInfo.class), EscapeField(eventInfo.rank), EscapeField(eventInfo.rankBefore),
            EscapeField(eventInfo.rankAfter), tostring(eventInfo.levelBefore or ""), tostring(eventInfo.levelAfter or ""),
            tostring(eventInfo.milestone or ""), eventInfo.reviewed and "1" or "0",
        }, "|"))
    end
...
function OTLGM:GetNextRecruitmentKey()
    self:EnsureDB()
    local index = OTLGM_DB.settings.nextRecruitIndex or 1
    local key = index == 1 and "BASE1" or "BASE2"
    OTLGM_DB.settings.nextRecruitIndex = index == 1 and 2 or 1
    return key
end
