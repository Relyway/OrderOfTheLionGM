...
local BaseScheduleMailboxR5 = OTLGM.ScheduleMailboxScan176
function OTLGM:ScheduleMailboxScan176(reason)
    self.runtime = self.runtime or {}
    self.runtime.mailScan176 = nil
    R5.mailboxScansSuppressed = R5.mailboxScansSuppressed + 1
    return false
end

local BaseProcessMailboxR5 = OTLGM.ProcessMailboxScan176
function OTLGM:ProcessMailboxScan176()
    if self.runtime then self.runtime.mailScan176 = nil end
    return false
end

local BaseBagSliceR5 = OTLGM.ProcessIncrementalBagScan176
if BaseBagSliceR5 then
    function OTLGM:ProcessIncrementalBagScan176()
        self.runtime = self.runtime or {}
        local hasWork = self.runtime.incrementalBagScan176 or self.runtime.incrementalBagDue176
        ...
        if tonumber(self.runtime.nextBagSliceR5 or 0) > now then
            R5.bagSlicesDeferred = R5.bagSlicesDeferred + 1
            return false
        end
        self.runtime.nextBagSliceR5 = now + BAG_SLICE_GAP_R5
        return BaseBagSliceR5(self)
    end
end

local BaseNetworkQueueR5 = OTLGM.ProcessNetworkQueue
if BaseNetworkQueueR5 then
    function OTLGM:ProcessNetworkQueue(maximum)
        ...
        return BaseNetworkQueueR5(self, limited)
    end
end
...
if BaseRefreshAchievementsR5 then
    function OTLGM:RefreshAchievements174()
        if not CanRefreshPageR5(self, "achievements") then return false end
        return BaseRefreshAchievementsR5(self)
    end
end

local BaseRefreshTreasuryR5 = OTLGM.RefreshTreasuryPage170
if BaseRefreshTreasuryR5 then
    function OTLGM:RefreshTreasuryPage170(forceEditor)
        if not CanRefreshPageR5(self, "treasury") then return false end
        return BaseRefreshTreasuryR5(self, forceEditor)
    end
end

local BaseRefreshAllR5 = OTLGM.RefreshAll
function OTLGM:RefreshAll()
    R5.refreshAllCollapsed = R5.refreshAllCollapsed + 1
    if not IsMainVisibleR5(self) then
        self.runtime = self.runtime or {}
        self.runtime.refreshAllDirtyR5 = true
        return false
    end
    if self.RefreshNavigation then self:RefreshNavigation() end
    if self.RefreshVisiblePage then self:RefreshVisiblePage() end
    if self.RefreshAddonUsersIndicator then self:RefreshAddonUsersIndicator() end
    if self.RefreshDateIndicator then self:RefreshDateIndicator(false) end
    if self.RefreshUpdateWarning then self:RefreshUpdateWarning() end
    self.runtime = self.runtime or {}
    self.runtime.refreshAllDirtyR5 = nil
    return true
end
...
function OTLGM:CloseExclusiveModalR5(frame)
    ...
    local active = self.ui.exclusiveModalR5
    if frame and active and frame ~= active then return end
    self.ui.exclusiveModalR5 = nil
    if active and active:IsVisible() then active:Hide() end
    if self.ui.exclusiveModalOverlayR5 then self.ui.exclusiveModalOverlayR5:Hide() end
end
...
local BaseOpenRecentWhispersR5 = OTLGM.OpenRecentWhispers176
if BaseOpenRecentWhispersR5 then
    function OTLGM:OpenRecentWhispers176()
        BaseOpenRecentWhispersR5(self)
        local dialog = self.ui and self.ui.recentWhisperDialog176
        if dialog then self:OpenExclusiveModalR5(dialog) end
    end
end

local BaseOpenContributionR5 = OTLGM.OpenTreasuryContributionDialog176
if BaseOpenContributionR5 then
    function OTLGM:OpenTreasuryContributionDialog176()
        BaseOpenContributionR5(self)
...
local BaseShowPageR5 = OTLGM.ShowPage
if BaseShowPageR5 then
    function OTLGM:ShowPage(pageKey)
        if self.ui and self.ui.exclusiveModalR5 then self:CloseExclusiveModalR5() end
        return BaseShowPageR5(self, pageKey)
    end
end
...
local BaseBuildWhispersR5 = OTLGM.BuildRecentWhisperDialog176
if BaseBuildWhispersR5 then
    function OTLGM:BuildRecentWhisperDialog176()
        BaseBuildWhispersR5(self)
        local dialog = self.ui and self.ui.recentWhisperDialog176
...
local BaseRefreshWhispersR5 = OTLGM.RefreshRecentWhispers176
if BaseRefreshWhispersR5 then
    function OTLGM:RefreshRecentWhispers176()
        BaseRefreshWhispersR5(self)
        local dialog = self.ui and self.ui.recentWhisperDialog176
        if not dialog then return end
        local entries = self.runtime and self.runtime.recentWhispers176 or {}
        local states = self.runtime and self.runtime.recentWhisperInviteStateR5 or {}
        local canInvite = self:CanInviteGuildMembersR5(false)
        local index, row, entry, state, member
        for index = 1, table.getn(dialog.rows176 or {}) do
            row = dialog.rows176[index]
            entry = entries[index]
            if row and row.invite176 then
                row.invite176.entryR5 = entry
                if entry then
                    row:Show()
                    row.invite176:Show()
                    member = self:IsGuildMemberNameR5(entry.name)
                    state = states[NameKeyR5(entry.name)]
                    if member or (state and state.state == "MEMBER") then
                        SetTextR5(row.invite176, "Member")
                        SetEnabledR5(row.invite176, false, "This player is already in the guild.")
                    elseif state and state.state == "SENT" and self:Now() - (tonumber(state.ts) or 0) < 60 then
                        SetTextR5(row.invite176, "Sent")
                        SetEnabledR5(row.invite176, false, "Invite was sent recently.")
                    else
                        SetTextR5(row.invite176, "Invite")
                        SetEnabledR5(row.invite176, canInvite and true or false, "Your current guild rank cannot invite members.")
                    end
                elseif index == 1 and table.getn(entries) == 0 then
                    row:Show()
                    if row.name176 then row.name176:SetText("No recent whispers yet") end
                    if row.snippet176 then row.snippet176:SetText("Incoming whisper senders will appear here for this session.") end
                    row.invite176:Hide()
                else
                    row.invite176:Hide()
                    row:Hide()
...
local BaseSetTreasuryGoalR5 = OTLGM.SetTreasuryGoal170
if BaseSetTreasuryGoalR5 then
    function OTLGM:SetTreasuryGoal170(id, name, current, target, category)
        ...
        local ok, result = BaseSetTreasuryGoalR5(self, id, name, current, target, category)
...
local BaseDeleteTreasuryGoalR5 = OTLGM.DeleteTreasuryGoal170
if BaseDeleteTreasuryGoalR5 then
    function OTLGM:DeleteTreasuryGoal170(id)
        local old = self.GetTreasuryGoal170 and self:GetTreasuryGoal170(id) or nil
        local ok, result = BaseDeleteTreasuryGoalR5(self, id)
        if ok then
            self:AddTreasuryActivityR5({
...
local BaseAddTreasuryContributionR5 = OTLGM.AddTreasuryContribution176
if BaseAddTreasuryContributionR5 then
    function OTLGM:AddTreasuryContribution176(goalId, contributor, amountCopper, note)
        self.runtime = self.runtime or {}
        self.runtime.recordingContributionR5 = true
        local ok, result = BaseAddTreasuryContributionR5(self, goalId, contributor, amountCopper, note)
...
local BaseHandleTreasuryR5 = OTLGM.HandleTreasuryMessage170
if BaseHandleTreasuryR5 then
    function OTLGM:HandleTreasuryMessage170(message, channel, sender)
        ...
        local handled = BaseHandleTreasuryR5(self, message, channel, sender)
...
function OTLGM:BuildTreasuryActivityR5()
    ...
    for index = 1, MAX_TREASURY_ACTIVITY_ROWS_R5 do
        ...
        row.textR5 = TextR5(row, "GameFontNormalSmall", "", 8, -7, 668, "LEFT")
...
function OTLGM:RefreshTreasuryActivityR5()
    ...
    SetEnabledR5(dialog.prevR5, dialog.offsetR5 > 0, "This is the first page.")
    SetEnabledR5(dialog.nextR5, dialog.offsetR5 < maximum, "There are no more entries.")
...
function OTLGM:BuildTreasuryGoalLedgerR5()
    ...
    for index = 1, MAX_LEDGER_SUMMARY_ROWS_R5 do
        ...
        row:SetWidth(230) row:SetHeight(37)
        BackdropR5(row, 6)
        row:SetBackdropColor(0.028, 0.027, 0.024, index / 2 == math.floor(index / 2) and 0.96 or 0.76)
        row:SetBackdropBorderColor(0.22, 0.20, 0.16, 1)
        row.nameR5 = TextR5(row, "GameFontNormalSmall", "", 8, -6, 138, "LEFT")
        row.amountR5 = TextR5(row, "GameFontNormalSmall", "", 148, -6, 72, "RIGHT")
        row.countR5 = TextR5(row, "GameFontNormalSmall", "", 8, -21, 212, "LEFT")
    ...
    for index = 1, MAX_LEDGER_ENTRY_ROWS_R5 do
        ...
        row:SetWidth(444) row:SetHeight(29)
        BackdropR5(row, 6)
        row:SetBackdropColor(0.028, 0.027, 0.024, index / 2 == math.floor(index / 2) and 0.96 or 0.76)
        row:SetBackdropBorderColor(0.22, 0.20, 0.16, 1)
        row.textR5 = TextR5(row, "GameFontNormalSmall", "", 8, -7, 428, "LEFT")
...
function OTLGM:RefreshTreasuryGoalLedgerR5()
    ...
    SetEnabledR5(dialog.summaryPrevR5, dialog.summaryOffsetR5 > 0, "This is the first contributor page.")
    SetEnabledR5(dialog.summaryNextR5, dialog.summaryOffsetR5 < summaryMaximum, "There are no more contributors.")
...
local BaseBuildTreasuryPageR5 = OTLGM.BuildTreasuryPage170
if BaseBuildTreasuryPageR5 then
    function OTLGM:BuildTreasuryPage170(page)
        local result = BaseBuildTreasuryPageR5(self, page)
...
local BaseRefreshTreasuryButtonsR5 = OTLGM.RefreshTreasuryPage170
function OTLGM:RefreshTreasuryPage170(forceEditor)
    if not CanRefreshPageR5(self, "treasury") then return false end
    local result = BaseRefreshTreasuryButtonsR5(self, forceEditor)
    local ui = self.ui and self.ui.treasury170
...
local BaseBuildUIR5 = OTLGM.BuildUI
if BaseBuildUIR5 then
    function OTLGM:BuildUI()
        local result = BaseBuildUIR5(self)
...
local BaseParkR5 = OTLGM.ParkWindow176
if BaseParkR5 then
    function OTLGM:ParkWindow176(side)
        local result = BaseParkR5(self, side)
        self:ApplyWindowTabLayoutR5()
        return result
    end
end

local BaseUnparkR5 = OTLGM.UnparkWindow176
if BaseUnparkR5 then
    function OTLGM:UnparkWindow176()
        local result = BaseUnparkR5(self)
        self:ApplyWindowTabLayoutR5()
        return result
    end
end
...
local BaseBuildRecruitmentR5 = OTLGM.BuildRecruitmentPage
if BaseBuildRecruitmentR5 then
    function OTLGM:BuildRecruitmentPage(page)
        local result = BaseBuildRecruitmentR5(self, page)
...
local BaseRefreshRecruitmentLayoutR5 = OTLGM.RefreshRecruitmentPage
if BaseRefreshRecruitmentLayoutR5 then
    function OTLGM:RefreshRecruitmentPage()
        if not CanRefreshPageR5(self, "recruitment") then return false end
        local result = BaseRefreshRecruitmentLayoutR5(self)
        self:ApplyRecruitmentCardLayoutR5()
        return result
    end
end
...
function OTLGM:ApplyGuildChatPolishR5()
    ...
    for index = 1, table.getn(self.ui.chatRows) do
        ...
        if row.messageFrame and row.chatData then
            ...
            row.messageFrame:SetWidth(achievement and 590 or (compact and 266 or 426))
        end
    end
end

local BaseRefreshGuildChatPolishR5 = OTLGM.RefreshGuildChatPage
if BaseRefreshGuildChatPolishR5 then
    function OTLGM:RefreshGuildChatPage()
        if not CanRefreshPageR5(self, "guildchat") then return false end
        local result = BaseRefreshGuildChatPolishR5(self)
...
if BaseRefreshHistoryRowsR5 then
...
local BaseRefreshOverviewCleanupR5 = OTLGM.RefreshOverviewPage
if BaseRefreshOverviewCleanupR5 then
    function OTLGM:RefreshOverviewPage()
        if not CanRefreshPageR5(self, "overview") then return false end
        local result = BaseRefreshOverviewCleanupR5(self)
...
local BaseQualityTimersR5 = OTLGM.ProcessQuality156Timers
function OTLGM:ProcessQuality156Timers()
    if BaseQualityTimersR5 then return BaseQualityTimersR5(self) end
end

-- Diagnostics preserve /otlperf while exposing the R5 counters.
local BasePerfSlashR5 = SlashCmdList and SlashCmdList["OTLGMPERF"]
if SlashCmdList then
    SlashCmdList["OTLGMPERF"] = function(message)
        ...
        if message == "reset" then
            ...
            if BasePerfSlashR5 then BasePerfSlashR5(message) end
            return
        end
        if BasePerfSlashR5 then BasePerfSlashR5(message) end
        if DEFAULT_CHAT_FRAME then
            DEFAULT_CHAT_FRAME:AddMessage("|cffffcc33[Lion GM R5]|r hidden refresh skips " .. tostring(R5.hiddenRefreshesSkipped) .. "; RefreshAll collapsed " .. tostring(R5.refreshAllCollapsed) .. "; mailbox scans suppressed " .. tostring(R5.mailboxScansSuppressed))
...
if OTLGM.RegisterModule then
    OTLGM:RegisterModule("Release176R5", {
        layer = "stability", revision = 5, version = "1.7.6", noOnUpdate = true,
