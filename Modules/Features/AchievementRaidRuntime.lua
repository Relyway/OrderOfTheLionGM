...
local function GetGuildLeader175(self)
    -- Guild-specific leader achievements intentionally trust only the two
    -- canonical character names.  This prevents a stale/corrupt rankIndex=0
    -- roster row from awarding leader achievements to an unrelated member.
    if self.GetCanonicalGuildLeaderName180 then
        return ShortName175(self:GetCanonicalGuildLeaderName180() or "")
    end
    return ""
end
...
local PreviousProgress175 = OTLGM.__impl180.GetAchievementProgress174__impl1
local PreviousPresentation175 = OTLGM.__impl180.GetAchievementPresentation174__impl1
...
function OTLGM.__impl180.GetAchievementProgress174__impl2(self, def)
    if not def then return 0, 1 end
    if def.id == "UNDER_BANNER" then
        return self:IsAchievementComplete174("UNDER_BANNER") and 1 or 0, 1
    end
    if string.sub(tostring(def.id),1,1) ~= "B" then return PreviousProgress175(self, def) end
...
function OTLGM.__impl180.GetAchievementPresentation174__impl2(self, def, complete)
    ...
    local name, description, _, secret = PreviousPresentation175(self, def, complete)
...
function OTLGM.__impl180.GetAchievementDisplayList174__impl2(self)
    ...
    local search = string.lower(Trim175((self.ui and self.ui.achievementSearchRuntime180) or OTLGM_DB.settings.achievementSearch174 or ""))
    local result = {}
...
function OTLGM.__impl180.BuildAchievementsPage174__impl2(self, page)
    ...
    self.ui.achievementSummaryTitle174=Text175(page,"GameFontNormalLarge","Guild Achievements",38,-2,430,"LEFT")
    ...
    self.ui.achievementSummarySubtitle174=Text175(page,"GameFontNormalSmall","Shared adventures, reliable milestones and personal guild history.",38,-29,540,"LEFT")
    self.ui.achievementSummarySubtitle174:SetTextColor(0.66,0.66,0.63)

    local progress = Panel175(page,38,-51,500,13,"background")
    self.ui.achievementProgressPanel174=progress
    ...
    self.ui.achievementCategories174=categories
    ...
    self.ui.achievementList174=list
...
function OTLGM.__impl180.RefreshAchievements174__impl2(self)
...
local PreviousComplete175 = OTLGM.__impl180.CompleteAchievement174__impl1
...
local function CheckMetaAchievements175(self, silent)
    ...
    if total >= 25 then PreviousComplete175(self,"B078",silent) end
    ...
    if count>=7 then PreviousComplete175(self,"B077",silent) end
    metaGuard175=false
end

function OTLGM.__impl180.CompleteAchievement174__impl2(self, id,silent)
    local changed=PreviousComplete175(self,id,silent)
...
local PreviousUpdateGroupSession175=OTLGM.__impl180.UpdateGroupSession174__impl1
function OTLGM.__impl180.UpdateGroupSession174__impl2(self, silent)
    local group=PreviousUpdateGroupSession175(self,silent)
...
function OTLGM.__impl180.HandleRelease175Message__impl1(self, message,channel,sender)
...
local PreviousStartBoss175=OTLGM.__impl180.StartBossEncounter174__impl1
local PreviousMarkBossDeath175=OTLGM.__impl180.MarkBossEncounterDeath174__impl1
local PreviousBossVictory175=OTLGM.__impl180.HandleBossVictory174__impl1
...
function OTLGM:StartBossEncounter174(bossName)
    local accepted=PreviousStartBoss175(self,bossName)
...
function OTLGM:MarkBossEncounterDeath174(name)
    PreviousMarkBossDeath175(self,name)
...
function OTLGM.__impl180.CheckResurrection175__impl1(self)
...
local function TrackRabbitTarget175(self)
    ...
    if RABBIT_NAMES_175[key] or (string.lower(tostring(creature))=="critter" and (string.find(key,"rabbit",1,true) or string.find(key,"hare",1,true))) then
        ...
        if self.SetRabbitCombatTracking175 then self:SetRabbitCombatTracking175(true) end
    else
        self.runtime.rabbitTarget175=nil
        if self.SetRabbitCombatTracking175 then self:SetRabbitCombatTracking175(false) end
    end
end
...
local function CheckRabbitDeath175(self,text)
    ...
    if string.find(string.lower(tostring(text or "")),string.lower(state.name),1,true) then
        self:CompleteAchievement174("B085",false)
        self.runtime.rabbitTarget175=nil
        if self.SetRabbitCombatTracking175 then self:SetRabbitCombatTracking175(false) end
    end
end
...
local PreviousCaptureGuildChat175=OTLGM.__impl180.CaptureGuildChatMessage__impl1
...
function OTLGM:CaptureGuildChatMessage(channel,message,sender)
    ...
    -- This table exists only to suppress a duplicate callback for the same
    -- physical chat packet.  The former three-second window also discarded
    -- legitimate repeated guild messages, so the addon could diverge from /g.
    -- GetTime is high-resolution on Interface 11200; if it is unavailable we
...
local PreviousUsefulActivity175=OTLGM.__impl180.AddUsefulActivity152__impl1
function OTLGM:AddUsefulActivity152(kind,title,detail,targetPage,timestamp,eventKey)
    if tostring(kind)=="REACTION" then
        ...
        local created=PreviousUsefulActivity175(self,kind,title,detail,targetPage,timestamp)
        if created and canonical~="" then created.eventKey175=canonical end
        return created
    end
    return PreviousUsefulActivity175(self,kind,title,detail,targetPage,timestamp)
end
...
local PreviousBuildActivity175=OTLGM.__impl180.BuildActivityPage__impl1
function OTLGM.__impl180.BuildActivityPage__impl2(self, page)
    PreviousBuildActivity175(self,page)
...
local PreviousCraftPanel175=OTLGM.__impl180.RefreshCraftingRecipesPanel__impl1
function OTLGM:RefreshCraftingRecipesPanel(summary)
    PreviousCraftPanel175(self,summary)
...
local PreviousBuildRaidEditor175=OTLGM.__impl180.BuildRaidEditor156__impl1

local PreviousOpenRaidEditor175=OTLGM.__impl180.OpenRaidEditor156__impl1

local PreviousPublishRaid175=OTLGM.__impl180.PublishPveRaidEvent156__impl1
function OTLGM:PublishPveRaidEvent156(data,existingId)
    local existing = existingId and self:GetRaidEvent180(existingId) or nil
    local oldStartTs = existing and tonumber(existing.startTs) or nil
    local ok,record=PreviousPublishRaid175(self,data,existingId)
    ...
    if self.ApplyRaidEventTargetingAfterPublish180 then
        local targetingOk,targetingError=self:ApplyRaidEventTargetingAfterPublish180(record,data,oldStartTs)
        if not targetingOk then return false,targetingError or "Raid targeting settings could not be saved." end
    else
...
function OTLGM:NotifyRaidInvites175(record,remote)
    ...
    -- The legacy path used to notify every client.  C6 owns eligibility by exact
    -- event roster and audience, so the old method only schedules that canonical
    -- evaluation and never creates a second toast/inbox entry.
    if self.ScheduleRaidEventAccessEvaluation180 then
        return self:ScheduleRaidEventAccessEvaluation180(record.id, remote and true or false)
    end
    return false
end

function OTLGM:ApplyRaidMeta157(fields, sender, channel)
    ...
    if sender and sender ~= "" then
        local allowed = record and self.IsRaidEventManager180 and self:IsRaidEventManager180(record, sender)
            or self:IsPveLeadershipName(sender) == true
...
function OTLGM:StartRaidInvites175(id)
    if self.StartRaidInviteCollection180 then return self:StartRaidInviteCollection180(id) end
    return false,"Invite Mode is unavailable."
end

local PreviousRaidEnhancements175=OTLGM.__impl180.BuildRaidEnhancements157__impl1
function OTLGM.__impl180.BuildRaidEnhancements157__impl2(self)
    PreviousRaidEnhancements175(self)
...
local PreviousRefreshRaid175=OTLGM.__impl180.RefreshRaidPlanner156__impl1
function OTLGM.__impl180.RefreshRaidPlanner156__impl2(self)
    PreviousRefreshRaid175(self)
    ...
    if raid then
        ...
        local dateLabel=start>0 and (self.FormatServerDate180 and self:FormatServerDate180(start,"%A, %d %B") or date("%A, %d %B",start)) or "Date to be announced"
...
local PreviousHomeRaid175=OTLGM.__impl180.RefreshHomePveSummary155__impl1
function OTLGM.__impl180.RefreshHomePveSummary155__impl2(self)
    PreviousHomeRaid175(self)
    ...
    local day=start>0 and (self.FormatServerDate180 and self:FormatServerDate180(start,"%A, %d %b") or date("%A, %d %b",start)) or "Date TBA"
...
local PreviousBuildToast175=OTLGM.__impl180.BuildAchievementToast174__impl1
function OTLGM:BuildAchievementToast174()
    PreviousBuildToast175(self)
    local index,toast
    for index=1,table.getn(self.ui and self.ui.guildToasts174 or {}) do
        ...
        if toast and not toast.releaseClick175 then
            ...
            toast:SetScript("OnClick",function()
                ...
                elseif data.targetPage and OTLGM.__impl180.ShowPage__impl1 then if OTLGM.ui.main and not OTLGM.ui.main:IsVisible() then OTLGM.ui.main:Show() end OTLGM:ShowPage(data.targetPage) end
...
local PreviousShowToast175=OTLGM.__impl180.ShowGuildToastNow174__impl1
function OTLGM:ShowGuildToastNow174(data,preferredIndex)
    local result=PreviousShowToast175(self,data,preferredIndex)
...
local PreviousApplyRemoteRaid175=OTLGM.__impl180.ApplyRemotePveRaid__impl1
function OTLGM:ApplyRemotePveRaid(fields)
    local result=PreviousApplyRemoteRaid175(self,fields)
    if result then
        ...
        local eventId=fields and fields[3]
        if eventId and self.ScheduleRaidEventAccessEvaluation180 then self:ScheduleRaidEventAccessEvaluation180(eventId,true) end
    end
    return result
end
...
function OTLGM:ProcessRelease175Timers()
    ...
    if self.runtime.rabbitTarget175 and now-(self.runtime.rabbitTarget175.ts or 0)>=45 then
        self.runtime.rabbitTarget175=nil
        if self.SetRabbitCombatTracking175 then self:SetRabbitCombatTracking175(false) end
    end
...
local PreviousQualityTimers175=OTLGM.__impl180.ProcessQuality156Timers__impl1
function OTLGM.__impl180.ProcessQuality156Timers__impl2(self)
    if PreviousQualityTimers175 then
        local ok, problem = pcall(PreviousQualityTimers175, self)
        if not ok and self.RecordInternalIssueRC3 then pcall(self.RecordInternalIssueRC3, self, "Quality/RELIABILITY", problem) end
    end
    if self.ProcessRelease175Timers then
        local ok, problem = pcall(self.ProcessRelease175Timers, self)
        if not ok and self.RecordInternalIssueRC3 then pcall(self.RecordInternalIssueRC3, self, "Quality/ACHIEVEMENT_RUNTIME", problem) end
    end
end

function OTLGM:RunRelease175LoginBaseline180()
    self:InstallEmoteHook175()
    self:InstallCraftHooks175()
...
-- RC3: rabbit combat parsing is dormant unless the player is actually targeting
-- a rabbit/hare for the secret achievement. These two events can be extremely
-- chatty in combat and should not reach Lua during ordinary play.
pcall(releaseEventFrame175.UnregisterEvent,releaseEventFrame175,"CHAT_MSG_COMBAT_SELF_HITS")
pcall(releaseEventFrame175.UnregisterEvent,releaseEventFrame175,"CHAT_MSG_SPELL_SELF_DAMAGE")
pcall(releaseEventFrame175.UnregisterEvent,releaseEventFrame175,"CHAT_MSG_COMBAT_HOSTILE_DEATH")
function OTLGM:SetRabbitCombatTracking175(active)
    if not releaseEventFrame175 then return false end
    if self.IsAchievementComplete174 and self:IsAchievementComplete174("B085") then active=false end
    if active then
        pcall(releaseEventFrame175.RegisterEvent,releaseEventFrame175,"CHAT_MSG_COMBAT_SELF_HITS")
        pcall(releaseEventFrame175.RegisterEvent,releaseEventFrame175,"CHAT_MSG_SPELL_SELF_DAMAGE")
        pcall(releaseEventFrame175.RegisterEvent,releaseEventFrame175,"CHAT_MSG_COMBAT_HOSTILE_DEATH")
        self.runtime=self.runtime or {}
        self.runtime.rabbitCombatTracking175=true
    else
        pcall(releaseEventFrame175.UnregisterEvent,releaseEventFrame175,"CHAT_MSG_COMBAT_SELF_HITS")
        pcall(releaseEventFrame175.UnregisterEvent,releaseEventFrame175,"CHAT_MSG_SPELL_SELF_DAMAGE")
        pcall(releaseEventFrame175.UnregisterEvent,releaseEventFrame175,"CHAT_MSG_COMBAT_HOSTILE_DEATH")
        if self.runtime then self.runtime.rabbitCombatTracking175=nil end
    end
    return true
end
releaseEventFrame175:SetScript("OnEvent",function()
    ...
    elseif event=="GUILD_ROSTER_UPDATE" or event=="PLAYER_GUILD_UPDATE" then
        ...
        if OTLGM.__impl180.RefreshAchievementRosterCache174__impl1 then OTLGM:RefreshAchievementRosterCache174(true) end
    ...
    elseif event=="PLAYER_TARGET_CHANGED" then
        -- This event already exists for the rabbit achievement, so reuse the
        -- same constant-time target change to improve Activity faction coverage
        -- without adding another listener or scan.
        if UnitName and UnitFactionGroup and OTLGM.RememberRosterFaction180 then
            local targetName=UnitName("target")
            if targetName and targetName~="" then
                local targetFaction=UnitFactionGroup("target")
                if targetFaction then OTLGM:RememberRosterFaction180(targetName,targetFaction,"target") end
            end
        end
        if not (OTLGM.IsAchievementComplete174 and OTLGM:IsAchievementComplete174("B085")) then
            TrackRabbitTarget175(OTLGM)
        elseif OTLGM.SetRabbitCombatTracking175 then
            OTLGM:SetRabbitCombatTracking175(false)
        end
