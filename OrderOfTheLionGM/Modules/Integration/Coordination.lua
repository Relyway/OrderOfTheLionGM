...
-- Cross-domain coordination: operations, raid planning and shared activity.
...
local function QPanel(parent, x, y, width, height)
    ...
    if OTLGM.ApplyPanelSkin then OTLGM:ApplyPanelSkin(frame, "surface") end
    return frame
end
...
local function QSetButton(button, text, enabled, reason, selected)
    ...
    OTLGM:SetControlEnabled170(button, enabled ~= false, reason)
...
local function QSetOperationButton156(button, text, state, reason)
    if not button then return end
    state = state or "IDLE"
    if button.text and button.text.SetText then button.text:SetText(text or "") end
    if button.label156 and button.label156.SetText then button.label156:SetText(text or "") end
    ...
    if state == "WORKING" then
        ...
        if button.text and button.text.SetTextColor then button.text:SetTextColor(0.70, 0.88, 1.0) end
    elseif state == "DONE" then
        ...
        if button.text and button.text.SetTextColor then button.text:SetTextColor(0.66, 1.0, 0.70) end
    elseif state == "ERROR" then
        ...
        if button.text and button.text.SetTextColor then button.text:SetTextColor(1.0, 0.58, 0.48) end
    else
        button:Enable()
        ...
        elseif button.SetBackdropColor then
            ...
            if button.text and button.text.SetTextColor then button.text:SetTextColor(0.61, 0.79, 1.0) end
        end
    end
end
...
function OTLGM:StoreSharedActivity156(bucket, online, source, received, deferRefresh)
    ...
    local db = self:GetGuildDB()
    local rosterMaximum = math.max(1, tonumber(db and db.lastTotal) or 0)
    if rosterMaximum <= 1 then rosterMaximum = 200 end
    online = math.max(0, math.min(rosterMaximum, tonumber(online) or 0))
...
function OTLGM:RecordSharedActivity156(force)
    ...
    -- Never publish a previous-session online count. A confirmed scan will call
    -- this method again after current roster data is available.
    if not db.lastScan or self:Now() - (tonumber(db.lastScan) or 0) > 1800 or (tonumber(db.lastTotal) or 0) <= 0 then return false end
...
function OTLGM:RequestSharedActivitySync156(force)
    ...
    self:QueueCommunityPayload(table.concat({ "S2", "SYNC", tostring(now), self.version or "?" }, "^"), "GUILD", nil, 1, "activity:sync")
    return true
end
...
function OTLGM:HandleSharedActivityMessage156(message, channel, sender)
    ...
    if fields[2] == "ACT" then return self:StoreSharedActivity156(fields[3], fields[4], QActivitySourceId(sender), true) end
    if fields[2] == "DAY" and protocol == "S2" then
        local dayStart = tonumber(fields[3]) or 0
        local sourceId = QActivitySourceId(sender)
...
function OTLGM:GetSharedActivityStats156(days)
    ...
    for ts, item in pairs(shared.buckets or {}) do
        if tonumber(ts) >= cutoff and item.count and item.count > 0 then
            local value = QMedianActivity156(item)
            sum = sum + value
            count = count + 1
...
function OTLGM:BuildActivityPage(page)
    ...
    -- Coverage already belongs to the 7-day average card. The old duplicate
    -- footer crossed into Guild Composition on narrow/effective UI scales.
...
function OTLGM:RepairCraftingProfessionIndex156(craft)
    ...
    for characterName, character in pairs(craft.characters) do
        if type(character) ~= "table" then
            craft.characters[characterName] = nil
        else
            local rebuilt = {}
            if type(character.professions) ~= "table" then character.professions = {} end
            for oldKey, profession in pairs(character.professions) do
                if type(profession) == "table" then
                    local key = self:NormalizeProfessionKey156(oldKey, profession.label)
                    if key ~= "" then
                        profession.key = key
                        if type(profession.recipes) ~= "table" then profession.recipes = {} end
                        if rebuilt[key] and rebuilt[key] ~= profession then
                            local recipeKey, recipe
                            if type(rebuilt[key].recipes) ~= "table" then rebuilt[key].recipes = {} end
                            for recipeKey, recipe in pairs(profession.recipes) do rebuilt[key].recipes[recipeKey] = recipe end
                            if (profession.ts or 0) > (rebuilt[key].ts or 0) then rebuilt[key].ts = profession.ts end
                        else
                            rebuilt[key] = profession
                        end
                    end
                end
            end
...
function OTLGM:BuildProfessionQOL156(page)
    ...
    local filterPanel = QPanel(recipes, 10, -108, 304, 160)
    ...
    local levelValues = { "ANY", "1_20", "21_40", "41_60", "61_PLUS", "UNKNOWN" }
    local skillValues = { "ANY", "1_75", "76_150", "151_225", "226_300", "301_PLUS", "UNKNOWN" }
    ...
    self.ui.qBasis170 = QButton(filterPanel, "Level type", 10, -34, 136, 26, function()
        local current = OTLGM_DB.settings.craftingLevelBasis170 or "ITEM"
        if current == "ITEM" then current = "REQUIRED" elseif current == "REQUIRED" then current = "SKILL" else current = "ITEM" end
        OTLGM_DB.settings.craftingLevelBasis170 = current
        OTLGM_DB.settings.craftingLevelFilter153 = "ANY"
        OTLGM:RefreshProfessionsPage()
    end, "utility")
    self.ui.qLevel156 = QButton(filterPanel, "Level", 154, -34, 136, 26, function()
        Cycle("craftingLevelFilter153", OTLGM_DB.settings.craftingLevelBasis170 == "SKILL" and skillValues or levelValues)
    end, "utility")
    self.ui.qRarity156 = QButton(filterPanel, "Rarity", 10, -66, 88, 26, function() Cycle("craftingRarityFilter153", rarityValues) end, "utility")
    self.ui.qSort156 = QButton(filterPanel, "Sort", 106, -66, 88, 26, function() Cycle("craftingSort153", sortValues) end, "utility")
    self.ui.qOnline156 = QButton(filterPanel, "Online first", 202, -66, 88, 26, function()
        OTLGM_DB.settings.craftingOnlineOnly153 = not OTLGM_DB.settings.craftingOnlineOnly153
        OTLGM:RefreshProfessionsPage()
    end, "confirm")
    self.ui.qReset156 = QButton(filterPanel, "Reset all filters", 10, -98, 280, 26, function()
        OTLGM_DB.settings.craftingLevelFilter153 = "ANY"
    ...
    QWrapped(filterPanel, "GameFontNormalSmall", "Exact metadata only. Unknown values are never guessed.", 10, -130, 280, 22):SetTextColor(0.50, 0.50, 0.48)
...
function OTLGM:RefreshProfessionQOL156()
    ...
    local basisLabel = basis == "SKILL" and "Skill" or basis == "REQUIRED" and "Use level" or "Item level"
    local basisShort = basis == "SKILL" and "Skill" or basis == "REQUIRED" and "Use" or "iLvl"
    local levelShort = level == "ANY" and "Any" or string.gsub(level, "_", "-")
    local rarityShort = rarity == "ANY" and "Any quality" or string.lower(rarity)
    local label = basisShort .. " " .. levelShort .. " - " .. rarityShort .. " - " .. online
    if self.ui.craftingFilterSummary156 then self.ui.craftingFilterSummary156:SetText(label) end
    QSetButton(self.ui.qBasis170, "Type: " .. basisLabel, true)
    QSetButton(self.ui.qLevel156, "Range: " .. level, true)
...
function OTLGM:EnsureRaid156DB()
    ...
    if type(pve.pastRaids156) ~= "table" then pve.pastRaids156 = {} end
    if type(pve.cancelledRaids156) ~= "table" then pve.cancelledRaids156 = {} end
    return pve
end
