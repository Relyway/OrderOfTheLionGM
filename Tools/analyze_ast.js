...
let defs=new Map(), calls=new Map(), methods=new Map();
...
function visit(n,file){if(!n||typeof n!=='object')return;if(n.type==='FunctionDeclaration'){let name=memberName(n.identifier);if(name)add(defs,name,{file,loc:n.loc&&n.loc.start.line})}if(n.type==='CallExpression'||n.type==='TableCallExpression'||n.type==='StringCallExpression'){let name=memberName(n.base);if(name)add(calls,name,{file,loc:n.loc&&n.loc.start.line})}if(n.type==='CallExpression'&&n.base&&n.base.type==='MemberExpression'&&n.base.indexer===':')add(methods,n.base.identifier.name,{file,loc:n.loc&&n.loc.start.line});for(const [k,v] of Object.entries(n)){if(k==='loc'||k==='range')continue;if(Array.isArray(v))for(const x of v)visit(x,file);else if(v&&typeof v==='object')visit(v,file)}}
