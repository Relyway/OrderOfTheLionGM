...
def main() -> int:
    ...
    for path in lua_files:
        ...
        for index, line in enumerate(lines):
            ...
            if not re.search(rf"PrepareInteractiveControl170\(\s*{target}\s*,", following):
                unprepared_controls.append(f"{path.relative_to(root)}:{index + 1}")
    check("all raw UI controls explicitly prepared", not unprepared_controls, ", ".join(unprepared_controls))
    ...
    check("all UI generations share enabled-state setter",
          ...
          "SetControlEnabled170(button, enabled, reason)" in pages_ui and
          "SetControlEnabled170(button, enabled, reason)" in experience_ui and
          "SetControlEnabled170(button, enabled ~= false, reason)" in coordination_ui)
    check("runtime schema constant", "OTLGM.schemaVersion = 14" in bootstrap and "local ROOT_SCHEMA = 14" in database)
    check("root shape repair installed", "EnsureRootShape170" in database and "EnsureGuildContainers170" in database)
    check("Favorites included in crafting cache key", "craftingFavoritesOnly170" in search)
    check("level basis included in crafting cache key", "craftingLevelBasis170" in search)
    check("temporary status expiry is heartbeat driven", "ProcessStatus170" in events)
    check("network queue supports coalescing", "coalesceKey" in transport)
    check("targeted non-recipient skip metric", "targetedSkipped" in security)
    check("transport is sole addon-message sender", combined.count("pcall(SendAddonMessage") == 1)
    check("outbound chat escape sanitizer installed", "NormalizeWirePayload172" in transport and "outboundSanitized172" in transport)
    check("presence protocol no longer sends raw pipes", '"V|" .. self.version' not in combined and '"Q|" .. self.version' not in combined)
    check("announcement read receipts installed", "RecordAnnouncementReadReceipt172" in combined and "readersButton172" in combined)
