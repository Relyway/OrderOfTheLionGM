...
local BaseShowPage = OTLGM.ShowPage
local BaseRefreshVisiblePage = OTLGM.RefreshVisiblePage
local BaseRefreshHomePage = OTLGM.RefreshHomePage
local BaseRefreshOverviewPage = OTLGM.RefreshOverviewPage
local BaseRefreshPvePage = OTLGM.RefreshPvePage
local BaseRefreshSettingsPage = OTLGM.RefreshSettingsPage
local BaseRefreshAll = OTLGM.RefreshAll
local BaseRefreshWizard = OTLGM.RefreshWizard
...
local function NSolid(parent, layer, r, g, b, a)
...
function OTLGM:BuildNextSearchPage(page)
    ...
    page:SetScript("OnUpdate", function()
        if not OTLGM.ui.searchDirty then return end
        OTLGM.ui.searchDirtyElapsed = (OTLGM.ui.searchDirtyElapsed or 0) + (arg1 or 0)
        if OTLGM.ui.searchDirtyElapsed >= 0.25 then
            OTLGM.ui.searchDirty = nil
            OTLGM.ui.globalSearchOffset = 0
            OTLGM:RefreshSearchPage(true)
        end
    end)
end
...
function OTLGM:BuildRecipesPanel(page)
    ...
    recipes:SetScript("OnUpdate", function()
        if not OTLGM.ui.craftingSearchDirty then return end
        OTLGM.ui.craftingSearchElapsed = (OTLGM.ui.craftingSearchElapsed or 0) + (arg1 or 0)
        if OTLGM.ui.craftingSearchElapsed >= 0.25 then
            OTLGM.ui.craftingSearchDirty = nil
            OTLGM.ui.craftingRecipeOffset = 0
            OTLGM.ui.craftingSelectedRecipe = nil
            OTLGM:RefreshProfessionsPage()
        end
...
function OTLGM:RefreshCraftingRecipesPanel(summary)
    ...
    for i = 1, 3 do
        ...
        if reagent then
            ...
            if not texture and tonumber(reagent.itemId) and tonumber(reagent.itemId) > 0 and GetItemInfo then local _, _, _, _, _, _, _, _, cached = GetItemInfo(reagent.itemId) texture = cached end
            material.icon:SetTexture(texture or "Interface\\Icons\\INV_Misc_QuestionMark")
            material.text:SetText(tostring(reagent.count or 0) .. "x  " .. NShort(reagent.name or "Unknown reagent", 23))
            material:Show()
        else material:Hide() end
    end
...
function OTLGM:ReactToBoard(reaction)
    local id = self.ui and self.ui.pveSelectedBoardPost
    if not id then return end
    self:SetCommunityReaction("BOARD", id, reaction, false)
    self:RefreshPvePage()
end
...
function OTLGM:BuildUI()
    ...
    if requestedPage == "search" or requestedPage == "professions" then self:ShowPage(requestedPage)
...
function OTLGM:RefreshNavigation()
    ...
    cursor = cursor - 28
    if guildOpen then
        local guildKeys = { "roster", "professions", "activity" }
        local i
    ...
    PlaceNavigationButton("settings", -484, 26)
    if self.ui.modeText then
        self.ui.modeText:ClearAllPoints()
        self.ui.modeText:SetPoint("TOPLEFT", self.ui.sidebar, "TOPLEFT", 12, -420)
    ...
    if self.ui.versionText then
        ...
        self.ui.versionText:SetPoint("TOPLEFT", self.ui.sidebar, "TOPLEFT", 12, -436)
    ...
    if self.ui.addonUsersButton then
        ...
        self.ui.addonUsersButton:SetHeight(24)
        self.ui.addonUsersButton:Show()
    end
    if self.ui.scanButton then
        self.ui.scanButton:ClearAllPoints()
        self.ui.scanButton:SetPoint("TOPLEFT", self.ui.sidebar, "TOPLEFT", 12, -516)
    ...
    local homeUnread = (self.GetNotificationUnread152 and (self:GetNotificationUnread152("announcement") + self:GetNotificationUnread152("reaction")) or 0)
    ...
    if self.GetNotificationUnread152 then craftUnread = craftUnread + self:GetNotificationUnread152("crafting") + self:GetNotificationUnread152("response") end
    ...
    NSetSelected(self.ui.guildSectionButton, visibleSelection == "roster" or visibleSelection == "professions" or visibleSelection == "activity")
...
function OTLGM:ShowPage(pageKey)
    ...
    if pageKey ~= "search" and pageKey ~= "professions" then
...
function OTLGM:ShowAddonUsersTooltip(owner)
    ...
    if table.getn(online) > 0 then
        ...
        local maxOnline = math.min(15, table.getn(online))
    ...
    local remaining = math.max(0, 18 - math.min(15, table.getn(online)))
...
local BaseBuildRecipesPanel153 = OTLGM.BuildRecipesPanel
local BaseBuildNextUI153 = OTLGM.BuildNextUI
local BaseGetCraftingSearchResults153 = OTLGM.GetCraftingSearchResults
local BaseRefreshCraftingRecipesPanel153 = OTLGM.RefreshCraftingRecipesPanel
...
local levelFilters153 = {
    ...
    {"41_59", "Level: 41-59"}, {"60", "Level: 60"}, {"UNKNOWN", "Level: ?"},
}
...
local function NGetItemMeta153(recipe)
    ...
    if itemId > 0 and GetItemInfo then
        local name, link, cachedQuality, cachedItemLevel, cachedRequiredLevel, cachedType, cachedSubType, stackCount, cachedEquipLoc, texture = GetItemInfo(itemId)
    ...
    return quality, requiredLevel, itemLevel, itemType, itemSubType, equipLoc
end
...
local function NMatchesLevel153(requiredLevel, filter)
    requiredLevel = tonumber(requiredLevel) or 0
    if filter == "ANY" then return true end
    if filter == "UNKNOWN" then return requiredLevel <= 0 end
    if filter == "1_20" then return requiredLevel >= 1 and requiredLevel <= 20 end
    if filter == "21_40" then return requiredLevel >= 21 and requiredLevel <= 40 end
    if filter == "41_59" then return requiredLevel >= 41 and requiredLevel <= 59 end
    if filter == "60" then return requiredLevel >= 60 end
    return true
end
...
function OTLGM:GetCraftingSearchResults(query, professionFilter)
    ...
    local filtered = {}
    local i, result, quality, requiredLevel, itemLevel, itemType, itemSubType, equipLoc, hasOnline, j
    for i = 1, table.getn(results) do
        result = results[i]
        quality, requiredLevel, itemLevel, itemType, itemSubType, equipLoc = NGetItemMeta153(result.recipe)
        hasOnline = false
        for j = 1, table.getn(result.crafters or {}) do if result.crafters[j].online then hasOnline = true break end end
        if (category == "ALL" or self:GetCraftingCategory153(result) == category) and NMatchesLevel153(requiredLevel, levelFilter) and NMatchesRarity153(quality, rarityFilter) and (not onlineOnly or hasOnline) then
    ...
    table.sort(filtered, function(a, b)
        ...
        if sortKey == "LEVEL" and (a.filterRequiredLevel153 or 0) ~= (b.filterRequiredLevel153 or 0) then return (a.filterRequiredLevel153 or 0) > (b.filterRequiredLevel153 or 0) end
...
function OTLGM:BuildRecipesPanel(page)
    ...
    self.ui.craftingLevelFilter153 = NButton(recipes, "Level: Any", 10, -78, 76, 26, function()
        NCycleSetting153("craftingLevelFilter153", levelFilters153)
        OTLGM.ui.craftingRecipeOffset = 0
        OTLGM:RefreshProfessionsPage()
    end, "utility")
...
local function NRefreshCategoryControls153(self)
    ...
    NSetButtonText(self.ui.craftingLevelFilter153, NFilterLabel153(levelFilters153, OTLGM_DB.settings.craftingLevelFilter153 or "ANY"))
